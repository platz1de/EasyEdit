	...
	public function split(): array
	{
		...
		for ($x = ($this->pos1->getX()) >> 4; $x <= ($this->pos2->getX()) >> 4; $x += 3) {
			for ($z = ($this->pos1->getZ()) >> 4; $z <= ($this->pos2->getZ()) >> 4; $z += 3) {
				$piece = new StaticBlockListSelection($this->getPlayer(), $level, new Vector3(max($x << 4, $this->pos1->getX()), max($this->pos1->getY(), 0), max($z << 4, $this->pos1->getZ())), min(48, $this->pos2->getX() - $x * 16 + 1, 48 + $x * 16 - $this->pos1->getX(), $this->pos2->getX() + 1 - $this->pos1->getX()), min($this->pos2->getY(), Level::Y_MASK) - $this->pos1->getY() + 1, min(48, $this->pos2->getZ() - $z * 16 + 1, 48 + $z * 16 - $this->pos1->getZ(), $this->pos2->getZ() + 1 - $this->pos1->getZ()), true);
