...
class Cube extends Selection implements Patterned
{
	...
	private const STRUCTURE_BLOCK = 0xFF0;
	...
	public function update(): void
	{
		if ($this->isValid()) {
			...
			if (!$this->piece && ($player = Server::getInstance()->getPlayer($this->player)) instanceof Player) {
				...
				PacketUtils::sendFakeBlock($this->structure->floor(), $this->getWorld(), $player, self::STRUCTURE_BLOCK, CompoundTag::create()
	...
	public function close(): void
	{
		if (!$this->piece && ($player = Server::getInstance()->getPlayerExact($this->player)) instanceof Player) {
			...
			PacketUtils::sendFakeBlock($this->structure, $this->getWorld(), $player, self::STRUCTURE_BLOCK);
