...
class StackInsertCommand extends EasyEditCommand
{
	...
	public function process(Session $session, array $args): void
	{
		...
		$session->runTask(new StackTask(new StackedCube($selection->getWorldName(), $selection->getPos1(), $selection->getPos2(), ArgumentParser::parseDirectionVector($session, $args[0] ?? null, $args[1] ?? null)), true));
	}
}