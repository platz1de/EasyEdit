...
class FillCommand extends EasyEditCommand
{
	...
	public function process(Session $session, array $args): void
	{
		...
		$session->runTask(new FillTask($session->asPlayer()->getWorld()->getFolderName(), $session->asPlayer()->getPosition()->asVector3(), ArgumentParser::parseFacing($session, $args[1] ?? null), $block));
	}
}